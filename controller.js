            ...
            async (error, stdout, stderr) => {
              ...
              function upgradePoll(stats) {
                total_owner =
                  ...
                  stats.selections[3];
                return {
                  holding_rate: `${((total_owner / stats.total) * 100).toFixed(
                    2
                  )}`,
                  non_elite: `${(
                    (stats.selections[1] / total_owner) *
                    100
                  ).toFixed(2)}`,
                  elite_one: `${(
                    (stats.selections[2] / total_owner) *
                    100
                  ).toFixed(2)}`,
                  elite_two: `${(
                    (stats.selections[3] / total_owner) *
                    100
                  ).toFixed(2)}`,
                };
              }
              ...
              function skillPoll(stats) {
                return {
                  unspecialized: `${(
                    (stats.selections[0] / stats.total) *
                    100
                  ).toFixed(2)}`,
                  specialization_one: `${(
                ...
                return Object.assign(
                  ...
                  upgradeData,
                  {
                    skills: item.skills.map((skill, index) => {
                      const skillData = skillPoll(
                        opB6DetailData.stats[skill.id]
                      );
                      const skillId =
                        CharCnInfo[indexCharInfo].skills[index].skillId;
                      return Object.assign(
                        {
                          id: skill.id,
                          type: index + 1,
                    ...
                    modules: item.equipments.map((module) => {
                      const moduleData = modulePoll(
                        opB6DetailData.stats[module.id]
                      );
                      const moduleId = Object.values(
                        moduleCnInfoData.equipDict
                      ).find(
